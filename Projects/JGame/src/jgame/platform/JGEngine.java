...
public abstract class JGEngine extends MIDlet implements JGEngineInterface {
        ...
        public void drawAll() {
            try {
				...
				/*
                     ...
                     * drawRect(bufg,(int)(viewWidth()*0.1),
                     * (int)(viewHeight()*(0.6+0.046)), (int)(viewWidth()*0.8),
                     * (int)(viewHeight()*0.008), true,false, false);
                     */
     ...
     */
    /**
     * Constructor called by application manager. Call initEngineApplet here.
     */
    public JGEngine() {
    }
    ...
    /**
     * This method is not used for midlets, since the window size is determined
     * by the environment.
     */
    ...
    /**
     * This method is not used for midlets, since in MIDP there is no such thing
     * as window layout.
     */
     ...
     * methods. In MIDP, we don't have reflection, so we only support a fixed
     * number of methods. Override this to add new methods.
     */
    public void tryMethod(String prefix, String suffix) {
        int prefidx, sufidx;
        for (prefidx = 0; prefidx < stateprefixes.length; prefidx++) {
            if (stateprefixes[prefidx].equals(prefix)) {
                break;
            }
        }
        for (sufidx = 0; sufidx < statesuffixes.length; sufidx++) {
            if (statesuffixes[sufidx].equals(suffix)) {
                break;
            }
        }
    ...
    public void setColor(JGColor col) {
        if (buf_gfx != null) {
            buf_gfx.setColor(col.r, col.g, col.b);
        }
    }
     ...
     */
    /**
     * MIDP has different mouse semantics, based on touch screen. Touch screens
     * have only press, drag, and release (no mouse move without button press).
     * There is only one "button", button 1. Mouseinside is set to false when
     * finger/stylus is not on the touch screen.
     */
     ...
     * getKeyDesc. The string is trimmed and lowercased. Non-static version for
...
//	} }
    ...
    /**
     * Short and fast Math.atan2 replacement. Average numerical error is less
     * than 0.001 radians, maximum error is 0.005 radians.
     */
    ...
    public void playAudio(String channel, String clipid, boolean loop) {
        ...
        try {
            if (cur_player != null) {
                ...
                if (cur_player.getState() == Player.STARTED) {
                    return;
                }
                ...
                if (cur_player != players.get(clipid)) {
            ...
            if (cur_player != null) {
                ...
                try {
                     ...
                     * Vairy Touch II when starting audio just after it was
                     * stopped
                     */
                }
