...
public abstract class StdGame extends JGEngine {
    ...
    public void setHighscores(int nr_highscores, Highscore default_hisc,
        ...
        for (int i = 0; i < nr_highscores; i++) // XXX maybe clone?
        {
    ...
    /**
     * Initialise the game when a new game is started. Default sets stage, score
     * to 0, and lives to initial_lives. Level is set to supplied argument.
     */
     ...
     * reinitialise the player. If you want a specific initialisation at both
    ...
    public void doFrameEnterHighscore() {
        /*
         * this method needs to be replaced. char key = getLastKeyChar();
         * clearLastKey(); if (key==KeyBackspace && playername.length()>0)
         * playername = playername.substring(0,playername.length()-1); if
         * (key==KeyEnter) { highscores = Highscore.insert(highscores, new
         * Highscore(score,playername)); clearLastKey(); clearKey(KeyEnter); //
         * save not implemented //saveHighscores(); seqtimer=0;
         * setGameState("Highscores"); } if (key>=32 && key<127 &&
         * playername.length()<highscore_maxnamelen) playername += key;
         */
    }
    ...
    public void paintFrameTitle() {
        drawString(getClass().getName().substring(getClass().getName().lastIndexOf('.') + 1),
    ...
    public void drawWavyString(String s, int x, int y, int align, int increment_x,
        ...
        for (int i = 0; i < s.length(); i++) {
            drawString(s.substring(i, i + 1), x + i * increment_x,
                    y + (int) (amplitude
                    * -Math.cos(Math.PI * (pos_phaseshift * i + tmr * timer_phaseshift))), 0);
        }
    }
