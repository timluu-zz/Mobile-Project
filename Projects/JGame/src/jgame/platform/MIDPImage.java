        ...
        for (int i = 0; i < dh; i++) {
            ...
            for (int j = 0; j < dw; j++) {
                do {
                    ...
                    // see also scale()
					/*
                     * int pixnr=0; int dp=0; if (pix1 > 0xffffff) { dp += pix1;
                     * pixnr++; } if (pix2 > 0xffffff) { dp += pix2; pixnr++; }
                     * if (pix3 > 0xffffff) { dp += pix3; pixnr++; } if (pix4 >
                     * 0xffffff) { dp += pix4; pixnr++; } if (pixnr<4) break; if
                     * ((dp&0xff000000) != 0) dp |= 0xff000000; dstData[wpos] = (dp/pixnr)<<2;
                     */
                    if (pix1 <= 0xffffff) {
                        pix1 = bg_pix;
                    }
