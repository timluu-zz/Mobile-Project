...
/**
 * A timer that generates a callback after a certain number of frames. It can
 * conveniently be used as a "one liner class", i.e. as an inner class within a
 * context where it can set a variable or call a method. i.e.
 * <code> new JGTimer (10,true) { alarm() { doSomething(); } };</code>
 *
 * <P>Timers are updated just before the beginning of a frame. Any objects they
 * add or delete are immediately updated before the frame starts.
 *
...
public abstract class JGTimer {
    ...
    /**
     * Create timer; the timer may be one-shot (it runs only once, then triggers
     * the alarm and removes itself), or continuous (it continues running and
    ...
    /**
     * Tick function, as called by the engine implementation. Returns true when
     * the timer has to be removed.
     */
    public boolean tick(double speed) {
        ...
        if (parent_state != null
                && !eng.inGameStateNextFrame(parent_state)) {
            return true;
        }
        if (parent_obj != null && !parent_obj.isAlive()) {
            return true;
        }
        if (running) {
            if (frames_left <= 0) {
