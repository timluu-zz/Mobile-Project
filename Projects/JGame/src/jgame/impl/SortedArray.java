...
public class SortedArray {
    ...
    public int get(String key) {
        /*
         * // algorithm 2 (wikipedia) int low = 0; int high = size; while (low
         * < high) { int mid = (low + high)/2; if (keys[mid].compareTo(key) < 0)
         * { low = mid + 1; } else { high = mid; //can't be high = mid-1: here
         * keys[mid] >= value, // so high can't be < mid if keys[mid] == value }
         * } if (low < size && keys[low].equals(key)) return low; else return
         * -1-low;
         */
        /*
         * // algorithm 1 (wikipedia) int low = 0; int high = size-1; while
         * (low <= high) { int mid = (low + high) / 2; int cmp =
         * keys[mid].compareTo(key); if (cmp>0) { high = mid - 1; else if
         * (cmp<0) { low = mid + 1; else return mid; } return -1;
         */
