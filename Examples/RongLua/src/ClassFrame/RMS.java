...
public class RMS {
    ...
    private static final int NUM_RECORD           = 1;
    ...
    public void open() {
        try {
            ...
            if(rs.getNumRecords() >= 0) {
                Player p ;
                p = new Player(null, Resource.IMG_SMALL_NUMBER, 0, 5, 0, Player.MAX_TIME, 1, "Quyet");
                insert(p);
            }
        }
    ...
    public void replaceWith(final Player p){
        try {             
                bis = new ByteArrayInputStream(rs.getRecord(0));
                ...
                if( score < p.getScore() ) {
                    ...
                    for(int i = RMS.NUM_RECORD; i >= 0+1; --i) {
                    ...
                    rs.setRecord(0, writeByte, 0, writeByte.length);                   
                }            
        }
        catch(IOException ioe) {
            System.out.println("Can't write data to RMS");
