...
//        canvas.getImage().drawImage(g, canvas, Resource.IMG_BG_OTHER, Constant.menu_x, Constant.menu_y);
        ...
        canvas.getImage().drawImage(g, canvas, Resource.IMG_BACK, (Constant.SCR_W - canvas.getResource().getIMG(Resource.IMG_BACK).getWidth()) - 35, (Constant.SCR_H - canvas.getResource().getIMG(Resource.IMG_BACK).getHeight()) - 20);
...
//            g.drawString(((Player)v.elementAt(i)).getName(), 70, 80+25*i, Graphics.TOP|Graphics.LEFT);
        int maxScore=((Player)v.elementAt(0)).getScore();
        String strMaxScore=String.valueOf(maxScore);
        for(int i=0;i<strMaxScore.length();i++)
        {   
            int indexNum=Integer.parseInt(strMaxScore.substring(i, i+1));
            canvas.getImage().drawImagePart(g, canvas, Resource.IMG_NUMBER, 120+i*20, 120, indexNum*20, 0, 20, 28);            
        }
        
//            g.drawString(strMaxScore, 120, 120, Graphics.TOP|Graphics.LEFT);
//        }
    }
}
