 ...
 * @author QuyetNM1
 */
public class Guide extends IScreen{
//    private Button buttBack = new Button("Quay lại", Resource.IMG_BUTTONLT, Resource.IMG_TXT_BUTTONLT, 2, Constant.buttBack_x, Constant.buttBack_y, Constant.buttonlt_w, Constant.buttonlt_h, Button.ENABLE);
    public Guide(CanvasGame _canvas) {
        super(_canvas);
    }

    public void load_screen() {
        canvas.getResource().loadArray(canvas.getResource().guideArrayImages);
    }

    public void un_load_screen() {
        canvas.getResource().unLoadArray(canvas.getResource().guideArrayImages);
    }
...
//        canvas.getImage().drawImageNormal(g, canvas, Resource.IMG_BG_OTHER, Constant.menu_x, Constant.menu_y);
        ...
        canvas.getImage().drawImage(g, canvas, Resource.IMG_TXT_GUIDE, (Constant.SCR_W - canvas.getResource().getIMG(Resource.IMG_TXT_GUIDE).getWidth())/2, Constant.txt_guide_y);

        canvas.getImage().drawImage(g, canvas, Resource.IMG_GUIDE, (Constant.SCR_W - canvas.getResource().getIMG(Resource.IMG_GUIDE).getWidth())/2, Constant.guide_y);
