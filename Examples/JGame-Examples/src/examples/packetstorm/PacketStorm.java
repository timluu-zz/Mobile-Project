...
public class PacketStorm extends StdGame {
    ...
    public static void main(String[] args) {
        new PacketStorm(parseSizeArgs(args, 0));
    }
    ...
    public void initGame() {
        ...
        setFrameRate(35, 1);
        key_startgame = KeyStar;
        key_continuegame = KeyFire;
    ...
    public void doFrameInGame() {
        ...
        if (gametime >= 2500 && countObjects("packet", 0) == 0) {
            levelDone();
        }
        if (getMouseButton(1)) {
    ...
    void flipSegment(int xseg, int yseg) {
        if (xseg < 0 || xseg >= nrxsegs || yseg < 0 || yseg >= nrysegs) {
            return;
        }
        ...
        for (int x = 0; x <= xsegsize; x += xsegsize) {
            for (int yi = 1; yi < ysegsize; yi++) {
                if (left + x <= 0 || left + x >= nrxsegs * xsegsize) {
                    continue;
                }
                if (and(getTileCid(left + x, top + yi), 1)) {
                    oncount++;
                }
        ...
        for (int y = 0; y <= ysegsize; y += ysegsize) {
            for (int xi = 1; xi < xsegsize; xi++) {
                if (top + y <= 0 || top + y >= nrysegs * ysegsize) {
                    continue;
                }
                if (and(getTileCid(left + xi, top + y), 1)) {
                    oncount++;
                }
    ...
    public class Cursor extends JGObject {
        ...
        public void move() {
            ...
            if (getKey(key_down) && y < pfHeight() - 16 - yspeed) {
                ydir = 1;
            }
    ...
    public class Packet extends JGObject {
        ...
        boolean checkAhead(int tx, int ty, int xdir, int ydir) {
            if (and(getTileCid(tx, ty), 1)) {
                return false;
            }
            if (and(getTileCid(tx, ty), 2)) {
                return true;
            }
            if (and(getTileCid(tx, ty), 4)) {
                return false;
            }
            if (tx <= 0 || tx >= nrxsegs * xsegsize) {
                return false;
            }
            if (ty <= 0 || ty >= nrysegs * ysegsize) {
                return false;
            }
            if (traillen == 100) {
                return false;
            ...
            if (waittime <= 0) {
                if (isXAligned() && isYAligned()) {
                    if (cen.x <= 0 || cen.x >= nrxsegs * xsegsize
                            || cen.y <= 0 || cen.y >= nrysegs * ysegsize) {
                        lifeLost();
                    }
                    snapToGrid();
