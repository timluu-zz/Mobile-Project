...
public class PubMan extends StdGame {
    ...
    public static void main(String[] args) {
        new PubMan(parseSizeArgs(args, 0));
    }
    ...
    public void paintFrame() {
        ...
        for (int i = 1; i < lives; i++) {
            drawImage(pfWidth() - 16 - 16 * i, 0, "player_d7");
        }
    }
    ...
    void floodFillPills(int x, int y) {
        if (and(getTileCid(x, y), 1 + 2 + 8)) {
            return;
        }
        if (!and(getTileCid(x, y), 4 + 32)) {
            setTile(x, y, "o");
        }
        if (x > 0) {
            floodFillPills(x - 1, y);
        }
        if (y > 0) {
            floodFillPills(x, y - 1);
        }
        if (x < pfTilesX() - 1) {
            floodFillPills(x + 1, y);
        }
