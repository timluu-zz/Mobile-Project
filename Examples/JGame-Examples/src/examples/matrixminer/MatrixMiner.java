...
public class MatrixMiner extends StdGame {
    ...
    public void doFrameInGame() {
        ...
        if (checkTime((int) (gametime < 30 + 4 * level ? 5 : 55 - level * 3))) {
            ...
            switch ((int) random(0, 4)) {
                case 0:
                    new Enemy(enemy_gfx, (int) random(1, 19, 2), -0.8, 0, 1);
                    break;
                case 1:
                    new Enemy(enemy_gfx, (int) random(1, 19, 2), 14.8, 0, -1);
                    break;
                case 2:
                    new Enemy(enemy_gfx, -0.8, (int) random(1, 15, 2), 1, 0);
                    break;
                case 3:
                    new Enemy(enemy_gfx, 18.8, (int) random(1, 15, 2), -1, 0);
                    break;
            }
        }
    ...
    public void paintFrame() {
        ...
        for (int i = 1; i < lives; i++) {
            drawImage(pfWidth() - 16 - 16 * i, 0, "player_r0");
        }
    }
    ...
    public class Goodie extends JGObject {
        ...
        public void hit(JGObject obj) {
            if (colid == 32) { // hit with enemy
                ...
                int xdir = 0, ydir = 0;
                if (player.xdir != 0 && player.y > y + 8) {
                    ydir = -1;
                }
                if (player.xdir != 0 && player.y < y - 8) {
                    ydir = 1;
                }
                if (player.ydir != 0 && player.x > x + 8) {
                    xdir = -1;
                }
                if (player.ydir != 0 && player.x < x - 8) {
                    xdir = 1;
                ...
                for (int i = 0; i < prevpos.length; i++) {
                    prevpos[i] = new JGPoint();
                }
                for (int i = 0; i < prevpos.length; i++) {
                    shiftPos(obj.x, obj.y);
                }
                score += 10;
                playAudio("pickup");
            }
