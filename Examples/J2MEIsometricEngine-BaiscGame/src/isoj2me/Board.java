...
public class Board {
	...
	public Board(String tileSet, String mapFile, int screenWidth, int screenHeight, 
...
//		}		
	...
	private void drawBoard(Graphics g, Element element, 
	    ...
	    do
	    {
	        ...
	        if (posOnCellsY >= 0 && posOnCellsY < map.height) {
            	...
            	if (drawOnlyElements) {
	        		for (posOnCellsX = edgeAX; posOnCellsX <= edgeBX; posOnCellsX ++, 
            			posOnMapX += hCellWidth, posOnMapY += hCellHeight) {
	        			if (posOnCellsX >= 0 && posOnCellsX < map.width) {
	        				currElement = isElement(posOnCellsX, posOnCellsY);
	        				if (currElement != null) {
	        						currElement.draw(g);
	        				}
	        				if ((posOnCellsX >= 0 && posOnCellsX < map.width) && !isWalkableCell(posOnCellsX, posOnCellsY)) { //TODO
	        		...
	        		for (posOnCellsX = edgeAX; posOnCellsX <= edgeBX; posOnCellsX ++, 
	            		posOnMapX += hCellWidth, posOnMapY += hCellHeight) {
	        			//|| elementsPositions.containsKey(posOnCellsX + "_" + posOnCellsY)
	        			if ((posOnCellsX >= 0 && posOnCellsX < map.width)) { //TODO
	    					map.setCellDeltaHeight(posOnCellsX, posOnCellsY, 
	    							drawTile(g, posOnCellsX, posOnCellsY, posOnMapX, posOnMapY, false));	        				
	        			}
	        		}
	...
	/**
	 * If there is a Element in the x and y position of the Board it returns
	 * its name. Return null if there is no element.
	 * 
	 * @param cellX
	 *            x
	 * @param cellY
	 ...
	 * @param c
	 *            Elenent to move
	 * @param posOnCellsNewX
	 *            new pos x on cells
	 * @param posOnCellsNewY
	 *            new pos y on cells
	 * @param overwrite
	 *            true if you want to overwrite an existing Element that stays
	 *            in the new position
