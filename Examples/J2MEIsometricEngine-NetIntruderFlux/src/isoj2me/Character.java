...
public class Character {
  ...
  public int lastDrawnX = 0;
  public int lastDrawnY = 0;
  ...
  /**
   * This draw the character at given coordinates on given Graphics. This method also change the current frame (if more than one) for the current action.
   * @param x x
   * @param y y
  ...
  public void draw(int x, int y, Graphics g) {
    ...
    if (currentAction != null && actions.containsKey(currentAction)) {
      ...
      if (frame > Integer.parseInt(actions.get(currentAction).toString()) || speed == 0) {
    	  frame = 0;
      }
      if (frame > 0) {
	tempFrame = frame + "";
  ...
  /**
   * This return the name that identify the action that is performed in this moment by the character
  ...
  /**
   * Set the current action for this character. The action have to be already in the actions list
